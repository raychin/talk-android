...
class MainActivity :
    ...
    private fun handleIntent(intent: Intent) {
        ...
        Log.d("OtaUpgradeWorker", "=== initWorkers start ===")
        // 每次启动都强制重新创建 OTA 检查任务
        val workManager = WorkManager.getInstance(applicationContext)
        // 强制取消所有 OTA 相关任务
        workManager.cancelAllWorkByTag("ota_upgrade")
        // 创建新的 OTA 升级检查任务
        val otaUpgradeWork = OneTimeWorkRequest.Builder(OtaUpgradeWorker::class.java)
            .addTag("ota_upgrade")
            .build()
        // 使用固定名称 + REPLACE 策略，确保每次都执行
